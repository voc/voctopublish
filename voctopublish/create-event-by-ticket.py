...
class RelivePublisher:
    ...
    def __init__(self, args = {}):
        ...
        try:
            ...
            self.c3tt = C3TTClient( self.config['C3Tracker']['url'], #get_var('url', 'CRS_TRACKER'), # CRS_TRACKER is https://tracker.c3voc.de/rpc but we need it without the rpc at the end, so do not use it for now... --Andi
    ...
    def create_event(self, ticket_id):
        ...
        if ticket._get_bool("Publishing.Voctoweb.EnableProfile"):
            ticket.voctoweb_event_id = ticket._get_str("Voctoweb.EventId", optional=True)
            ...
            ticket.links = ticket._get_list("Fahrplan.Links", optional=True, split_by=" ")
            ticket.people = ticket._get_list("Fahrplan.Person_list")
            ticket.license = ticket._get_str("Meta.License", optional=True, try_default=True)
    ...
    def _publish_event_to_voctoweb(self, ticket):
        ...
        if not(ticket.voctoweb_event_id):
            ...
            r = requests.get(f"{self.config['voctoweb']['frontend_url']}/public/events/{ticket.voctoweb_event_id}")
