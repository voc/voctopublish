...
class VoctowebClient:
    def __init__(self, t: Ticket, thumb: ThumbnailGenerator, api_key, api_url, ssh_host, ssh_port, ssh_user, frontend_url=None):
        """
        :param t:
        :param thumb:
    ...
    def get_event(self):
        ...
        logging.debug("api url: " + url + ' header: ' + str(headers) + ' slug: ' + str(self.t.slug))

        # call voctoweb api
        r = requests.get(self.frontend_url + '/public/events/' + self.t.voctoweb_event_id, headers=headers)
        return r.json()
